...
function UserKycDetails({ userid }) {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const result = await axios.get(`${baseUrl}/admin/dashboard/view/kyc/detail?userId=${userid}&page=${currentPage}&limit=${itemsPerPage}&status=${requestType}`, {
          headers: {
        ...
        setTotalDocs(result?.data?.totalDocs || 0); // Set totalDocs from the response
      } catch (e) {
        console.log(e);
        setLoading(false);
      }
