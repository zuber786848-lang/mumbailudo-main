...
function UserGameHistory({userid}) {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const result = await axios.get(`${baseUrl}/admin/dashboard/view/game/history?userId=${userid}&page=${currentPage}&limit=${itemsPerPage}&status=${requestType}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token') || ''}`
          }
        });
        setLoading(false);
        setData(result?.data?.data?.userGameHistory);
