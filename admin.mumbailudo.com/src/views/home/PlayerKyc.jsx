...
function PlayerKyc() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const result = await axios.get(`${baseUrl}/admin/kyc/view/details`, {
          headers: {
                  ...
                  data?.map((item, index) => (
                    <tbody className="divide-y divide-gray-300">
                      ...
                      >
                        ...
                        <td onClick={() => navigate(`/player/details/${item.user_id._id}`)} className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 hover:text-indigo-500 cursor-pointer">
                          {item.user_id.name}
                        </td>
                        <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
            ...
            <div className="pagination flex justify-end mt-4">
              {Array.from({ length: totalPages }, (_, index) => (
                <button
                  ...
                  className={`mx-1 px-3 py-2 border rounded-md ${currentPage === index + 1
