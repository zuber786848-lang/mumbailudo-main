...
function  PlayerList() {
  ...
  async function searchPlayer(phone) {
    setLoading(true);
    try {
      const result = await axios.get(
        `${baseUrl}/admin/dashboard/users?page=${currentPage}&limit=${itemsPerPage}&phoneNo=${phone}`,
        {
          headers: {
      ...
      setLoading(false);
      console.log(result)
      setData(result?.data?.data?.searchedUsers);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const result = await axios.get(
          `${baseUrl}/admin/dashboard/users?page=${currentPage}&limit=${itemsPerPage}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token') || ''}`
            }
        ...
        setLoading(false);
        setData(result?.data?.data?.allUsers);
        setTotalDocs(result?.data?.data?.totalDocs || 0); // Set totalDocs from the response
      } catch (e) {
        console.log(e);
        setLoading(false);
                ...
                <div className="overflow-x-auto w-full  border rounded-xl border-gray-300">
                    ...
                    <thead>
                      <tr className="bg-gray-50">
                        ...
                        <th
                          scope="col"
                          className="p-1 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize "
                        >
                        Action
                        </th>
                    ...
                    {
                      data?.map((item, index) => (
                        <Player player={item} index={startIndex + index + 1} key={item._id}/>
                      ))
                    }
