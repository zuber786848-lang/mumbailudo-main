...
function  Transaction() {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const result = await axios.get(`${baseUrl}/admin/transaction/request/get?request_type=${requestType}&page=${currentPage}&limit=${itemsPerPage}`, {
        headers: {
                    ...
                    </thead>
                    {
                      data?.map((item, index) => (
                        ...
                        >
                            ...
                            <td  onClick={() => navigate(`/player/details/${item.user_id._id}`)} className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900  hover:text-indigo-600 cursor-pointer">
                                {item.user_id.name}
                            </td>
                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
