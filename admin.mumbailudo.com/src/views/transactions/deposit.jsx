...
function  TransactionDeposit() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      console.log(result?.data?.data?.requestGet)
  ...
  async function filterDepositTxn(transaction_id) {
    setLoading(true);
    try {
      const result = await axios.get(`${baseUrl}/admin/transaction/request/get?request_type=${requestType}&page=${currentPage}&limit=${itemsPerPage}&status=${selectedFilter}&transaction_id=${transaction_id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token') || ''}`
        }
      });
...
const handlePageChange = (pageNumber) => {
  ...
  async function approveRejectDeposit(id, status) {
    setLoading(true);
    try {
        let url = `${baseUrl}/admin/transaction/deposit/approved`;
        let requestData = { _id: id };

        if (status === "rejected") {
            url = `${baseUrl}/admin/transaction/deposit/rejected`;
...
useEffect(() => {
  ...
  const handleGameCreation = () => {
    ...
    if (Notification.permission === 'granted') {
      ...
      new Notification('Deposit Received', {
        body: 'A new deposit request has been received.',
        icon: '/transference.png', // Optional icon
      });
    }
  };

  socket.on('socket-deposit-receive-refresh', handleGameCreation);

  // Cleanup on component unmount
  return () => {
    socket.off('socket-deposit-receive-refresh', handleGameCreation); // Cleanup
  ...
  return (
    ...
    <div
      className="w-full lg:flex mt-3 lg:mt-0 item-center justify-between px-5  mb-4 "
      id="navbar-with-form"
    >
     <form className="flex justify-between items-center w-full xl:w-1/2 space-x-2">
      ...
      <button
        ...
        type="button"
        onClick={() =>
          filterDepositTxn(document.getElementById("default-search").value)
        }
      >
        <svg
                    ...
                    </thead>
                    {
                      data?.map((item, index) => (
                        ...
                        >
                            ...
                            <td className="p-5 capitalize whitespace-nowrap text-sm leading-6 font-bold text-rose-500">
                                {item.transaction_id}
                            </td>
                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                {item.deposit_amount ?? "N/A"}
                            </td>
